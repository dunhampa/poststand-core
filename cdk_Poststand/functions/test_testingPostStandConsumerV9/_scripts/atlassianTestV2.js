...
async function mainFunction() {
  ...
  const requestConfig = {
    method: 'GET',
    url: `https://${(await getSecret('secretOne.jiraDomain'))}/rest/api/2/issue/${globalGet('issueKey')}`,
    queryParams: {
  ...
  const authConfig = { type: 'basic', username: await getSecret('secretOne.jiraEmailAddress'), password: await getSecret('secretOne.jiraApiKey') };
...
globalSet("anotherResponse", response.data)
console.log("headersPrint", response.headers)
globalSet("anotherHeader", response.headers)
globalSet("Checking", "WTF")
  } catch (e) {
